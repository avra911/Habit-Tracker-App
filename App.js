...
export default function App() {
  ...
  useEffect(() => {
    if (!loading && !enableCircleView && view === 'tracker' && isCurrentMonth) {
      ...
      setTimeout(() => { horizontalScrollRef.current?.scrollTo({ x: Math.max(0, scrollPos), animated: true }); }, 500);
    }
  }, [loading, view, enableCircleView, isCurrentMonth]);
  ...
  useEffect(() => {
    ...
    saveLocal();
    const timer = setTimeout(() => { if (enableNostrSync && userPrivkey) publishToNostr(); }, 3000);
    return () => clearTimeout(timer);
  }, [dailyHabits, weeklyHabits, monthlyHabits, history, isDarkMode, enableCircleView, enableNostrSync]);

  useEffect(() => {
    if (enableNostrSync && userPrivkey && userPubkey) listenToNostr();
  ...
  const publishToNostr = async () => {
    ...
    try {
      ...
      const event = finalizeEvent({ kind: 30000, content: encryptedContent, tags: [['d', 'habit-tracker-sync']], created_at: Math.floor(Date.now() / 1000) }, userPrivkey);
      await Promise.allSettled(RELAYS.map(async (url) => {
        try { const r = new Relay(url); await r.connect(); await r.publish(event); r.close(); } catch (e) { }
      }));
          ...
          onevent: async (event) => {
            try {
              ...
              if (decrypted !== lastSyncedRef.current) {
                ...
                setDailyHabits(data.dailyHabits || []); setWeeklyHabits(data.weeklyHabits || []); setMonthlyHabits(data.monthlyHabits || []); setHistory(data.history || {});
  ...
  const copyToClipboard = async (text) => {
    if (Platform.OS === 'web') { if (navigator.clipboard) await navigator.clipboard.writeText(text); }
    else { await Clipboard.setString(text); }
  ...
  const importPrivateKey = async () => {
    ...
    if (input.length !== 64) { Alert.alert('Error', 'Invalid key'); return; }
    try {
      const privkey = hexToPrivkey(input);
      const pubkey = getPublicKey(privkey);
      await AsyncStorage.setItem('@orbit_v15_nostr_privkey', input);
      setUserPrivkey(privkey); setUserPubkey(pubkey); setImportPrivateKeyInput('');
            ...
            {enableNostrSync && (
              <>
                <View style={[styles.modernInputRow, { borderTopColor: theme.border, flexDirection: 'column', alignItems: 'flex-start', paddingVertical: 8 }]}>
                  ...
                  <View style={[styles.keyDisplay, { backgroundColor: theme.bg, borderColor: theme.border }]}><Text style={[styles.keyText, { color: theme.text }]}>{userPubkey}</Text></View>
                  <TouchableOpacity onPress={() => copyToClipboard(userPubkey)} style={[styles.copyBtn, { backgroundColor: habitColors[0] }]}><Text style={styles.copyBtnText}>📋 Copy Public Key</Text></TouchableOpacity>
                </View>
                <View style={[styles.dangerZone, { borderColor: theme.border, backgroundColor: theme.bg }]}>
                  ...
                  {!showPrivateKey ? (
                    <View style={[styles.keyDisplay, { backgroundColor: theme.bg, borderColor: theme.border, justifyContent: 'center', alignItems: 'center' }]}><Text style={[styles.keyText, { color: theme.subtext }]}>••••••••••••••••</Text></View>
                  ) : (
                    <View style={[styles.keyDisplay, { backgroundColor: theme.bg, borderColor: theme.border }]}><Text style={[styles.keyText, { color: theme.text }]}>{privkeyToHex(userPrivkey)}</Text></View>
                  )}
                  <View style={styles.keyButtonsRow}>
                    <TouchableOpacity onPress={() => setShowPrivateKey(!showPrivateKey)} style={[styles.halfBtn, { backgroundColor: theme.border, marginRight: 6 }]}><Text style={styles.halfBtnText}>{showPrivateKey ? '🙈' : '👁️'}</Text></TouchableOpacity>
                    <TouchableOpacity onPress={() => copyToClipboard(privkeyToHex(userPrivkey))} style={[styles.halfBtn, { backgroundColor: habitColors[0] }]}><Text style={styles.halfBtnText}>📋 Copy</Text></TouchableOpacity>
                  </View>
                  <TextInput placeholder="Paste key" placeholderTextColor={theme.subtext} value={importPrivateKeyInput} onChangeText={setImportPrivateKeyInput} style={[styles.importInput, { color: theme.text, borderColor: theme.border, backgroundColor: theme.bg }]} />
                  <TouchableOpacity onPress={importPrivateKey} style={[styles.importBtn, { backgroundColor: '#10b981' }]}><Text style={styles.importBtnText}>✓ Import & Sync</Text></TouchableOpacity>
                </View>
              </>
            )}
          </View>
            ...
            <View style={styles.layoutWrapper}>
                ...
                {(isDesktop && enableCircleView) ? (
                  <View style={styles.svgContainer}>
                    <Svg viewBox={`0 0 ${svgSize} ${svgSize}`} style={StyleSheet.absoluteFill}>
                      <Defs>{dailyHabits.map((_, h) => (<Path key={h} id={`textPath-${h}`} d={describeArcClockwise(cx, cy, outerR - h * ringWidth - ringWidth / 2, -60, 0)} />))}</Defs>
                      ...
                      {dailyHabits.map((name, h) => (
                        <G key={h}>
                          <SvgText fill={habitColors[h % 10]} fontSize="14" fontWeight="bold"><TextPath href={`#textPath-${h}`} startOffset="95%" textAnchor="end">{name || "Habit..."}</TextPath></SvgText>
                          {Array.from({ length: daysInMonth }).map((_, d) => {
                            const isFuture = isCurrentMonth && (d + 1) > today.getDate();
                            const isActive = currentMonthData.daily[`${d}-${h}`];
                            return <Path key={d} d={describeAnnularSector(cx, cy, outerR - (h + 1) * ringWidth, outerR - h * ringWidth, d * anglePerDay, (d + 1) * anglePerDay)}
                              fill={isActive ? habitColors[h % 10] : isFuture ? theme.card : theme.bg}
                              stroke={theme.card} strokeWidth="2" onPress={() => !isFuture && updateHistory('daily', `${d}-${h}`, !isActive)} opacity={isFuture ? 0.3 : 1} />;
                          })}
                        </G>
                      ))}
                        ...
                        <View>
                          <View style={[styles.mobileDayHeaderRow, { backgroundColor: theme.bg }]}>
                            {Array.from({ length: daysInMonth }).map((_, d) => {
                              ...
                              return <View key={d} style={[styles.mobileDayNumBox, { borderColor: theme.border }, isToday && { backgroundColor: habitColors[0] }]}><Text style={[styles.mobileDayNumText, isToday && { color: '#fff', fontWeight: 'bold' }]}>{d + 1}</Text></View>;
                            })}
                          </View>
                            ...
                            <View key={hIdx} style={[styles.mobileHabitRow, { borderColor: theme.border, backgroundColor: hIdx % 2 === 0 ? theme.card : theme.bg }]}>
                              {Array.from({ length: daysInMonth }).map((_, dIdx) => (
                                <TouchableOpacity key={dIdx} onPress={() => updateHistory('daily', `${dIdx}-${hIdx}`, !currentMonthData.daily[`${dIdx}-${hIdx}`])} disabled={isCurrentMonth && (dIdx + 1) > today.getDate()}
                                  style={[styles.mobileCell, { borderColor: theme.border, backgroundColor: currentMonthData.daily[`${dIdx}-${hIdx}`] ? habitColors[hIdx % 10] : (today.getDate() === dIdx + 1 && isCurrentMonth) ? 'rgba(59, 130, 246, 0.1)' : 'transparent' }, isCurrentMonth && (dIdx + 1) > today.getDate() && { opacity: 0.1 }]} />
                              ))}
                            </View>
                          ))}
                        </View>
              ...
              <View style={styles.bottomGridsColumn}>
                <View style={styles.gridSection}>
                  <View style={[styles.singleHeaderRow, { borderBottomColor: theme.border }]}><Text style={styles.smallHeading}>Weekly Goals</Text><View style={styles.headerWeekNums}>{Array.from({ length: weeksCount }).map((_, i) => <Text key={i} style={styles.weekLabel}>W{i + 1}</Text>)}</View></View>
                  <View style={styles.gridBody}>{weeklyHabits.map((h, hIdx) => (
                    <View key={hIdx} style={[styles.tableRow, { borderBottomColor: theme.border }]}><TextInput value={h} onChangeText={(t) => { let c = [...weeklyHabits]; c[hIdx] = t; setWeeklyHabits(c); }} style={[styles.tableInput, { color: theme.text }]} placeholder="Goal..." placeholderTextColor={theme.subtext} />
                      <View style={styles.rowCells}>{Array.from({ length: weeksCount }).map((_, w) => (<TouchableOpacity key={w} onPress={() => updateHistory('weekly', `${w}-${hIdx}`, !currentMonthData.weekly[`${w}-${hIdx}`])} style={[styles.cell, { borderColor: theme.border, backgroundColor: currentMonthData.weekly[`${w}-${hIdx}`] ? habitColors[hIdx % 10] : theme.bg }]} />))}</View></View>
                  ))}</View>
                </View>
                <View style={styles.gridSection}>
                  <View style={[styles.singleHeaderRow, { borderBottomColor: theme.border }]}><Text style={styles.smallHeading}>Monthly Milestones</Text></View>
                  <View style={styles.gridBody}>{monthlyHabits.map((h, hIdx) => (
                    <View key={hIdx} style={[styles.tableRow, { borderBottomColor: theme.border }]}><TextInput value={h} onChangeText={(t) => { let c = [...monthlyHabits]; c[hIdx] = t; setMonthlyHabits(c); }} style={[styles.tableInput, { color: theme.text }]} placeholder="Milestone..." placeholderTextColor={theme.subtext} />
                      <TouchableOpacity onPress={() => updateHistory('monthly', hIdx, !currentMonthData.monthly[hIdx])} style={[styles.cell, { width: 44, borderColor: theme.border, backgroundColor: currentMonthData.monthly[hIdx] ? habitColors[hIdx % 10] : theme.bg }]}><Text style={{ color: '#fff', fontWeight: 'bold' }}>{currentMonthData.monthly[hIdx] ? '✓' : ''}</Text></TouchableOpacity></View>
                  ))}</View>
                </View>
              </View>
            </View>
...
const lightTheme = { bg: '#f1f5f9', card: '#ffffff', text: '#0f172a', subtext: '#64748b', border: '#e2e8f0' };
